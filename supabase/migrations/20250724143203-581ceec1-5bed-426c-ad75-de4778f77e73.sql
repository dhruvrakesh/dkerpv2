-- Phase 1: Manufacturing Workflow Enhancement - Database Schema Extension
...
-- Create artwork management table
...
CREATE TABLE public.dkegl_production_metrics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  organization_id UUID REFERENCES public.dkegl_organizations(id),
  order_id UUID REFERENCES public.dkegl_orders(id),
  stage_id UUID REFERENCES public.dkegl_workflow_stages(id),
  ...
  efficiency_percentage NUMERIC GENERATED ALWAYS AS (
    CASE WHEN planned_quantity > 0 THEN (actual_quantity / planned_quantity * 100) ELSE 0 END
  ) STORED,
  ...
  cost_per_unit NUMERIC GENERATED ALWAYS AS (
    CASE WHEN actual_quantity > 0 THEN (material_cost + labor_cost + overhead_cost) / actual_quantity ELSE 0 END
  ) STORED,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
ALTER TABLE public.dkegl_production_metrics ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "DKEGL organization members can access production metrics" ON public.dkegl_production_metrics
  FOR ALL USING (organization_id = dkegl_get_current_user_org())
  WITH CHECK (organization_id = dkegl_get_current_user_org());
...
CREATE INDEX idx_production_metrics_order_id ON public.dkegl_production_metrics(order_id);
CREATE INDEX idx_production_metrics_date ON public.dkegl_production_metrics(metric_date);
