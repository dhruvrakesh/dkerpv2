-- Fix SQL function with correct table names and column references
...
-- 2. Create dkegl_get_comprehensive_stock_summary with correct table references
...
BEGIN
  ...
  WITH stock_summary AS (
    ...
    LEFT JOIN (
      SELECT organization_id as org_id, item_code, SUM(opening_qty) as opening_qty
    ...
    LEFT JOIN dkegl_categories cat ON cat.id = item.category_id AND cat.organization_id = item.org_id
...
-- 3. Create dkegl_reconcile_stock_data with correct references
