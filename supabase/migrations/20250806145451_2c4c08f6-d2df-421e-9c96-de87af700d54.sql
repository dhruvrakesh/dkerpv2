...
-- Create RLS policy for opening stock
...
-- Create RLS policy for reconciliation log
...
-- 5. Create function to get comprehensive stock summary with proper calculations
...
BEGIN
  ...
  WITH stock_calculations AS (
    SELECT 
      ...
      COALESCE(grn.total_received, 0) as total_grn_qty,
      COALESCE(iss.total_issued, 0) as total_issued_qty,
      s.current_qty,
      COALESCE(os.opening_qty, 0) + COALESCE(grn.total_received, 0) - COALESCE(iss.total_issued, 0) as calculated_qty,
      s.current_qty - (COALESCE(os.opening_qty, 0) + COALESCE(grn.total_received, 0) - COALESCE(iss.total_issued, 0)) as variance_qty,
      s.unit_cost,
      s.current_qty * s.unit_cost as total_value,
      s.last_transaction_date,
      CASE 
        WHEN s.current_qty <= 0 THEN 'Out of Stock'
        WHEN s.current_qty <= COALESCE(im.reorder_level, 10) THEN 'Low Stock'
        ELSE 'In Stock'
      END as stock_status,
      CASE 
        WHEN COALESCE(iss.avg_daily_consumption, 0) > 0 
        THEN (s.current_qty / iss.avg_daily_consumption)::INTEGER
        ELSE 999
      END as days_of_cover,
      COALESCE(im.reorder_level, 0) as reorder_level,
      s.current_qty <= COALESCE(im.reorder_level, 10) as is_low_stock
    FROM dkegl_stock s
    ...
    LEFT JOIN (
      SELECT item_code, SUM(opening_qty) as opening_qty
      FROM dkegl_opening_stock 
      WHERE organization_id = _org_id
      GROUP BY item_code
    ) os ON s.item_code = os.item_code
    LEFT JOIN (
      SELECT item_code, SUM(qty_received) as total_received
      FROM dkegl_grn_log 
      WHERE organization_id = _org_id
      GROUP BY item_code
    ) grn ON s.item_code = grn.item_code
    LEFT JOIN (
      SELECT 
        item_code, 
        SUM(qty_issued) as total_issued,
        AVG(qty_issued) FILTER (WHERE date >= CURRENT_DATE - INTERVAL '30 days') as avg_daily_consumption
      FROM dkegl_issue_log 
      WHERE organization_id = _org_id
      GROUP BY item_code
    ) iss ON s.item_code = iss.item_code
    WHERE s.organization_id = _org_id
  )
  SELECT * FROM stock_calculations
  ORDER BY stock_calculations.item_code;
END;
$$;