-- Fix the ambiguous column reference in comprehensive stock summary function
...
BEGIN
  ...
  SELECT 
    ...
    -- Opening stock (from stock table itself)
    s.opening_qty,
    ...
    s.opening_qty + COALESCE(grn_summary.total_grn_qty, 0) - COALESCE(issue_summary.total_issued_qty, 0) as calculated_qty,
    
    -- Variance (current - calculated)
    s.current_qty - (s.opening_qty + COALESCE(grn_summary.total_grn_qty, 0) - COALESCE(issue_summary.total_issued_qty, 0)) as variance_qty
  ...
  LEFT JOIN (
    ...
    WHERE grn.organization_id = _org_id
  ...
  LEFT JOIN (
    SELECT 
      iss.organization_id, 
      iss.item_code, 
      SUM(iss.qty_issued) as total_issued_qty
    FROM dkegl_issue_log iss
    WHERE iss.organization_id = _org_id
    GROUP BY iss.organization_id, iss.item_code
