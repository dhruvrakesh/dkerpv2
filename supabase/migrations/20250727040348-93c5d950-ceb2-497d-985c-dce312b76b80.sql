-- Phase 2B: Complete Material Flow Implementation
...
AND material_item_code IN ('PVDC_COAT_25_kg', 'NYL_FILM_12MIC_kg');
...
WHERE stage_id IS NULL 
AND material_item_code IN ('PU_ADH_2K_kg', 'SOL_MEK_kg');
...
BEGIN
  ...
  LEFT JOIN current_stock cs ON sr.material_item_code = cs.item_code
...
LEFT JOIN dkegl_stock s ON s.organization_id = wp.organization_id AND s.item_code = bc.material_item_code
WHERE NOT EXISTS (
  SELECT 1 FROM dkegl_stage_material_inputs smi 
  WHERE smi.workflow_progress_id = wp.id 
  AND smi.item_code = bc.material_item_code
);
...
SELECT DISTINCT
  ...
  wp.stage_id,
  wp.id,
  COALESCE(bc.material_item_code, 'SUBSTRATE_BASE') AS input_code,
...
-- Update material categories for consistency
UPDATE dkegl_stage_material_categories 
SET category_name = 'substrate_carryforward'
WHERE category_name = 'substrate' 
AND stage_id IN (
  SELECT id FROM dkegl_workflow_stages 
  WHERE stage_name NOT LIKE '%Slitting%'
);

UPDATE dkegl_stage_material_categories 
SET category_name = 'fresh_adhesive'
WHERE category_name = 'adhesive' 
AND stage_id IN (
  SELECT id FROM dkegl_workflow_stages 
  WHERE stage_name LIKE '%Adhesive%' OR stage_name LIKE '%Coating%'
);
