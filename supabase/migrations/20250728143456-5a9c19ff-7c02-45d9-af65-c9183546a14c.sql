...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM dkpkl_import_batches WHERE organization_id = org_id) AND org_id IS NOT NULL THEN
    ...
    INSERT INTO dkpkl_import_batches (
      ...
      0, 5, current_user_id, now() - interval '2 hours', now() - interval '1 hour',
      '{"description": "Sample sales data for testing dashboard", "source": "demo"}'::jsonb
    );
    ...
    SELECT 
      ...
      jsonb_build_object(
        ...
        'party_name', 'Customer ' || (generate_series(1, 150) % 20 + 1)
      ),
      'processed',
    ...
    INSERT INTO dkpkl_import_batches (
      ...
      2, 3, current_user_id, now() - interval '30 minutes',
      '{"description": "Sample purchase data - currently processing", "source": "demo"}'::jsonb
    );
    
    -- Insert sample raw rows for batch 2 (partial)
    INSERT INTO dkpkl_raw_rows (
      batch_id, organization_id, row_number, row_data, 
      parsed_status, posted_to_erp
    ) 
    SELECT 
      sample_batch_id_2,
      org_id,
      ...
      jsonb_build_object(
        ...
        'invoice_no', 'INV-' || LPAD(generate_series(1, 60)::text, 4, '0')
      ),
      'processed',
