-- Fix Migration: Handle Orphaned Stock Records and Execute Consolidation
-- Drop the unique constraint first to avoid conflicts
ALTER TABLE dkegl_stock DROP CONSTRAINT IF EXISTS dkegl_stock_org_item_location_unique;

-- Create a safe consolidation function that handles orphaned records
...
-- Now add the unique constraint to prevent future fragmentation
