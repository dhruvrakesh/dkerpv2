-- Fix GROUP BY issues in analytics functions and populate stock summary table

-- 1. Fix dkegl_get_inventory_analytics - GROUP BY clause issue with annual_turnover_ratio
...
-- 2. Fix dkegl_get_pricing_intelligence - GROUP BY clause issue with variance_percentage
...
-- 3. Fix dkegl_get_predictive_insights - GROUP BY clause issue with avg_monthly_demand
...
-- 4. Create procedure to populate stock summary table
...
-- 5. Populate stock summary for DKEGL organization
DO $$
DECLARE
  dkegl_org_id uuid;
  population_result jsonb;
BEGIN
  ...
  IF dkegl_org_id IS NOT NULL THEN
    ...
    SELECT dkegl_populate_stock_summary(dkegl_org_id) INTO population_result;
    RAISE NOTICE 'Stock summary populated: %', population_result;
  ELSE
