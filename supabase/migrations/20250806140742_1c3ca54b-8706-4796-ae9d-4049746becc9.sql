-- Fix SQL function parameter naming conflicts to resolve stock calculation issues

-- 1. Fix dkegl_get_comprehensive_stock_summary function with proper parameter naming
...
-- 2. Fix dkegl_reconcile_stock_data function with proper parameter naming
...
-- 3. Update dkegl_get_current_user_org function to use correct table names
CREATE OR REPLACE FUNCTION dkeglpkl_get_current_org_id()
RETURNS uuid
LANGUAGE plpgsql
...
BEGIN
  ...
  -- For now, always return DKEGL org since we don't have user profiles yet
  SELECT id INTO user_org_id 
  FROM dkeglpkl_org 
  WHERE code = 'DKEGL' 
  LIMIT 1;
