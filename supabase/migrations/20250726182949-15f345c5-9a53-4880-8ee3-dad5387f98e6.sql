-- Phase 1: Enhanced Material Flow Tables (Corrected)
...
-- Add material_category column to dkegl_bom_components if it doesn't exist
ALTER TABLE public.dkegl_bom_components 
ADD COLUMN IF NOT EXISTS material_category TEXT DEFAULT 'raw_material';
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((im.pricing_info->>'unit_cost')::NUMERIC, 0) as unit_cost,
    (bc.quantity_per_unit * order_qty * COALESCE((im.pricing_info->>'unit_cost')::NUMERIC, 0)) as total_planned_cost,
    (bc.quantity_per_unit * order_qty * bc.waste_percentage / 100) as waste_allowance,
    (bc.quantity_per_unit * order_qty * (1 + bc.waste_percentage / 100) * COALESCE((im.pricing_info->>'unit_cost')::NUMERIC, 0)) as total_with_waste
  ...
  ORDER BY bc.material_category, bc.component_item_code;
END;
$function$;