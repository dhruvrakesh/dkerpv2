-- Activate Stock Summary System with Comprehensive Sample Data (Final)
DO $$
DECLARE
  org_id UUID;
...
BEGIN
  ...
  -- Insert GRN Logs (Goods Received) with UOM
  INSERT INTO dkegl_grn_log (
    organization_id, grn_number, item_code, date, qty_received, uom, total_amount, supplier_name
  ) VALUES
  ...
  RAISE NOTICE 'Stock Summary System Successfully Activated! Items: %, GRN Records: %, Issue Records: %', 
    (SELECT COUNT(*) FROM dkegl_item_master WHERE organization_id = org_id),
    (SELECT COUNT(*) FROM dkegl_grn_log WHERE organization_id = org_id),
    (SELECT COUNT(*) FROM dkegl_issue_log WHERE organization_id = org_id);
END $$;