-- Activate Stock Summary System with Comprehensive Sample Data
-- Get organization ID
DO $$
DECLARE
  org_id UUID;
...
BEGIN
  ...
  -- Insert Categories
  INSERT INTO dkegl_categories (id, organization_id, category_name, description, is_active) VALUES
  (gen_random_uuid(), org_id, 'Packaging Materials', 'Flexible packaging and films', true),
  (gen_random_uuid(), org_id, 'Raw Materials', 'Base materials for production', true),
  (gen_random_uuid(), org_id, 'Consumables', 'Production consumables and supplies', true)
  ON CONFLICT DO NOTHING;

  -- Get category IDs
  SELECT id INTO cat_packaging_id FROM dkegl_categories WHERE organization_id = org_id AND category_name = 'Packaging Materials';
  SELECT id INTO cat_raw_material_id FROM dkegl_categories WHERE organization_id = org_id AND category_name = 'Raw Materials';
  SELECT id INTO cat_consumables_id FROM dkegl_categories WHERE organization_id = org_id AND category_name = 'Consumables';
  ...
  RAISE NOTICE 'Stock Summary System Activated Successfully with % items', (SELECT COUNT(*) FROM dkegl_item_master WHERE organization_id = org_id);
END $$;