-- Phase 1.2: Critical Function Fixes & Emergency Cleanup
...
DECLARE
  ...
  result jsonb;
BEGIN
  -- Step 1: Consolidate duplicate locations by item_code
  ...
  consolidation_inserts AS (
    ...
    SELECT 
      ...
      0, -- Will be updated with actual consolidated amount
      lc.total_qty,
  ...
  -- Step 2: Delete old location entries and insert consolidated ones
  ...
  SELECT 
    ...
    item_code,
    'MAIN-STORE',
    GREATEST(SUM(current_qty), 0), -- Ensure no negative after consolidation
    MAX(unit_cost),
  ...
  -- Step 4: Fix remaining negative stocks
...
-- Create missing dkegl_get_comprehensive_stock_summary function
...
-- Create missing dkegl_get_stock_analytics_totals function  
