-- Complete BOM Component Stage Assignments & Material Categories
-- Phase 1A: Fix Critical Database Structure Gaps

-- 1. Complete BOM component stage assignments
...
WHERE component_code = 'RAW_INK_001';

UPDATE dkegl_bom_components SET stage_id = (
  SELECT id FROM dkegl_workflow_stages WHERE stage_name = 'Gravure Printing' LIMIT 1
), material_category = 'solvents'
WHERE component_code = 'RAW_SOLVENT_001';
...
WHERE component_code = 'RAW_PRIMER_001';

UPDATE dkegl_bom_components SET stage_id = (
  SELECT id FROM dkegl_workflow_stages WHERE stage_name = 'Adhesive Coating' LIMIT 1
), material_category = 'specialized_adhesives'
WHERE component_code = 'RAW_COATING_001';

UPDATE dkegl_bom_components SET stage_id = (
  SELECT id FROM dkegl_workflow_stages WHERE stage_name = 'Slitting Packaging' LIMIT 1
), material_category = 'cores'
WHERE component_code = 'RAW_CORE_001';

-- 2. Create missing material categories for all stages
-- Order Punching stage materials
INSERT INTO dkegl_stage_material_categories (organization_id, stage_id, category_name, category_description, material_properties)
SELECT 
  (SELECT id FROM dkegl_organizations WHERE code = 'DKEGL' LIMIT 1),
  ws.id,
  unnest(ARRAY['substrate', 'tooling', 'consumables', 'setup_materials']),
  unnest(ARRAY[
    'Base substrate materials for order preparation',
    'Cutting tools and setup equipment',
    'General consumable materials',
    'Stage setup and preparation materials'
  ]),
  unnest(ARRAY[
    '{"typical_usage": "high", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "low", "waste_tracking": false}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "low", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "low", "waste_tracking": false}'::jsonb
  ])
...
SELECT 
  ...
  unnest(ARRAY[
    'Substrate for printing process',
    'Printing inks - all colors',
    'Ink solvents and thinners',
    'Printing plates and cylinders',
    'Process chemicals and additives',
    'Cleaning agents for equipment',
    'Cylinder preparation materials'
  ]),
  unnest(ARRAY[
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true, "color_tracking": true}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "high", "waste_tracking": false}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "low", "waste_tracking": false}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "low", "waste_tracking": false}'::jsonb
  ])
FROM dkegl_workflow_stages ws
...
SELECT 
  ...
  unnest(ARRAY[
    'Printed substrate from previous stage',
    'Lamination adhesives',
    'Surface primers and activators',
    'Release agents for process',
    'Catalysts for adhesive curing',
    'Process solvents'
  ]),
  unnest(ARRAY[
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true, "coverage_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "low", "waste_tracking": false}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "medium", "waste_tracking": true}'::jsonb
...
SELECT 
  ...
  unnest(ARRAY[
    'Laminated substrate from previous stage',
    'Specialized adhesive coatings',
    'Adhesive activators and crosslinkers',
    'Coating process chemicals',
    'Release papers and liners'
  ]),
  unnest(ARRAY[
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "high", "cost_impact": "high", "waste_tracking": true, "thickness_tracking": true}'::jsonb,
    '{"typical_usage": "low", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "medium", "waste_tracking": true}'::jsonb,
    '{"typical_usage": "medium", "cost_impact": "medium", "waste_tracking": false}'::jsonb
  ])
FROM dkegl_workflow_stages ws
...
-- 3. Add 50+ fresh materials to item master for all stages
