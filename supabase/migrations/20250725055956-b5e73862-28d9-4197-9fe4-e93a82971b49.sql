-- Create sample quality inspections with correct table structure
...
SELECT 
  ...
  now(),
  CASE s.stage_order 
    WHEN 1 THEN '{"overall_grade": "A-", "color_match": "within_tolerance", "defects": "minor_streak", "measurements": {"color_density": 1.45, "registration_accuracy": 0.08}}'
    ELSE '{"status": "pending", "measurements": {}}'
  END,
  CASE s.stage_order 
    WHEN 1 THEN 'passed'
    ELSE 'pending'
  ...
  END,
  CASE s.stage_order 
    WHEN 1 THEN '["Clean cylinder before next run", "Adjust ink viscosity"]'
    ELSE '[]'
  END,
  CASE s.stage_order 
